...
bool Text::create(TTF_TextEngine* engine, FontManager* pmgr, LockerIndex font_indx, const char* text) {
    ...
    pFont = pFontMgr->GetLockerResource(mFontIndex);
    mRect = { -1 };
...
bool Text::preprocess() {
    ...
    mText.text = TTF_CreateText(pEngine, pFont->getFont(), text, 0);
...
void Text::setFontMgrIndex(LockerIndex font_ind) noexcept {
    ...
    if (mRect.w < 0.f) return;
    mFontIndex = font_ind;
    pFont = mFontIndex == -1 ? pFontMgr->GetLockerResource(mFontIndex) : nullptr;
    if (pFont == nullptr) {
        Logger log(DEFAULT_LOG_PATH);
        log.logWarningIn(__FUNCTION__, "text got nullptr font.");
        mRect.w = -1.f;
        return;
    }
    if (!TTF_SetTextFont(mText.text, pFont->getFont())) {
        logSDLErr(__FUNCTION__);
        return;
    }
}
